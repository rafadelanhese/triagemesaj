...
public class ArquivoService {
        ...
        for (Processo processo : listaProcesso) {
            for (PalavraChave palavra : listaPalavraChave) {
                if (processo.getTeor().contains(palavra.getParametro()) && !classificado) {
                    listaProcClassificado.add(new ProcessoClassificado(processo.getNumeroProcesso(), processo.getForo(),
                            processo.getVara(), processo.getDisponibilizacao(), processo.getPrazoProcessual(),
                            processo.getMovimentacao(), processo.getTeor(), processo.getClasse(), processo.getAssunto(),
                            processo.getSituacao(), palavra.getModalidade(), palavra.getRepercussao(),
                            palavra.getPrazo(), palavra.getEtiqueta()));
    ...
    private void gravaListaNoArquivoTriado(List<ProcessoClassificado> listaProcClassificado) {
        try {
            ...
            writer.write(
                    "Número do processo;Foro;Vara;Disponibilização;Prazo processual;Movimentação;Teor da intimação;Classe principal;Assunto principal;Situação;MODALIDADE;REPERCUSSÃO;PRAZO;ETIQUETA\n");
    ...
    private void gravaListaNoArquivoNaoTriado(List<Processo> listaProcNaoClassificado) {
        try {
            ...
            writer.write(
                    "Número do processo;Foro;Vara;Disponibilização;Prazo processual;Movimentação;Teor da intimação;Classe principal;Assunto principal;Situação\n");
